import React from 'react';
import { BrowserRouter as Router, Route, Routes } from 'react-router-dom';
import './App.css';
import './ArticleList.css';

const Newsdetails2 = () => {
  return (
    <div>
    <h1>Noticias de Eventos Fotograficos</h1>
    <div class="article-list">
      
      
      <div class="article-cardn">
      <h1>Cuarta edición del Fotomatón Festival</h1>
      <img src="/assets/images/noticia11.png" alt="Notice 11" />
        <p className="primary-text">
     
        <br /><br />
        Las exposiciones este año serán:
        <br /><br />
        “The End” del estadounidense Rodney Smith cuyo humor, distinción y surrealismo exquisito le hicieron mundialmente famoso.
        <br /><br />
        “Disparando con el corazón”, una retrospectiva del trabajo de la que fue la primera fotoperiodista de nuestro país, Joana Biarnés. Personajes desde los Beatles a Dalí pasaron por el objetivo de esta sorprendente mujer.
        <br /><br />
        “Como si no hubiera un mañana” de Ricardo Cases y Martinho Costa. Esta exposición es una idea del fotógrafo Ricardo Cases cuya propuesta expositiva indaga sobre las limitaciones de la fotografía y la pintura como elementos de confrontación de la realidad.
        <br /><br />
        La cuarta exposición reflejará la obra ganadora del Portfolio Fotomatón en su segunda edición y en donde el autor premiado podrá exponer hasta un total de 25 obras.
                
        </p>

      </div>

      <div class="article-cardn">
      <h1>Apertura Inscripciones Premios LUX 2021</h1>
      <img src="/assets/images/noticia12.png" alt="Notice 12" />
        <p className="primary-text">
     
        <br /><br />
        La Asociación de Fotógrafos Profesionales de España, AFPE, lanza la 29ª edición de los Premios de Fotografía Profesional LUX y presenta la nueva Imagen LUX 2021, cuya autora es la Asociada Irene Sekulic.
        <br /><br />
        La convocatoria está abierta a todos los fotógrafos profesionales que ejerzan la profesión en el territorio español hasta el 1 de octubre.
        <br /><br />
        Después de batir récords de inscripciones durante los últimos años, los Premios LUX siguen consolidándose año tras año como un referente de la Fotografía Profesional en España.
                
        <br /><br />
               
        </p>

      </div>

      <div class="article-cardn">
      <h1>BFoto cierra su octava edición apoyando la difusión de los artistas emergentes</h1>
      <img src="/assets/images/noticia13.png" alt="Notice 13" />
        <p className="primary-text">
     
        <br /><br />
        La sobrina responsable del cuidado de Don Quijote, aquella muchacha que “no llegaba a los veinte” con un IPhone y una pompa de chicle o un grito de “mil veces no” tatuado en una pared al paso del río Vero son algunos de los rastros que ha dejado el Festival de Fotografía Emergente en las calles de Barbastro (BFoto) tras este fin de semana. Este encuentro, que apuesta por la gente que cree en lo que hace y quiere contarlo, ha celebrado el grueso de sus actividades durante los pasados 19,20 y 21 de agosto mediante la organización de exposiciones, charlas, un cine de verano y dos catas de vino del Somontano.
        <br /><br />
        Durante estas jornadas, el certamen se ha desarrollado con enorme participación ciudadana y ha convertido a la capital de la comarca oscense en un epicentro cultural donde los amantes de la fotografía han podido encontrar una gran variedad de iniciativas fotográficas. Por ello, la Asociación Fotográfica y de la Imagen de Barbastro (AFIB) – impulsora y organizadora del evento – hace un balance “plenamente satisfactorio” de este arranque del festival. Asimismo resaltan el aumento de propuestas recibidas durante esta edición: un total de 251 trabajos, de 40 nacionalidades diferentes en la categoría Emergentes y otros 149 en el apartado Open-Gran Formato.
                
        <br /><br />
               
        </p>

      </div>


      <div class="article-cardn">
      <h1>¡5 días! Red Bull Illume Image Quest 2021</h1>
      <img src="/assets/images/noticia14.png" alt="Notice 14" />
        <p className="primary-text">
     
        <br /><br />
        Red Bull Illume Image Quest 2021! © Michael Clark / Red Bull Illume
        <br /><br />
        Esta es la última llamada a todos los fotógrafos y videógrafos profesionales o aficionados de deportes de acción y aventura para que envíen sus trabajos.
        <br /><br />
        El plazo de presentación se acerca rápidamente: queda menos de una semana para que subas tus mejores fotos y vídeos antes del 31 de julio, ya sea a través de redbullillume.com o de Instagram utilizando el hashtag #rbi21submission y etiquetando a @redbullillume.
                
        <br /><br />
               
        </p>

      </div>


      <div class="article-cardn">
      <h1>Red Bull Illume: Medio mes para el final</h1>
      <img src="/assets/images/noticia15.png" alt="Notice 15" />
        <p className="primary-text">
     
        <br /><br />
        La cuenta atrás para el Red Bull Illume Image Quest 2021 ha comenzado, ya que se acerca el final de la fase de presentación de candidaturas. Solo queda medio mes para que los fotógrafos y videógrafos presenten sus mejores imágenes de aventura y deportes de acción para ganar prestigio, reconocimiento mundial, premios valorados en más de 200.000 euros y la oportunidad de que sus imágenes se publiquen en todo el mundo.
        <br /><br />
        El recordatorio de la fecha límite llega mientras revelamos algunos de los jueces que han aceptado ayudar a seleccionar a los ganadores. Entre ellos se encuentra Russ O’Connell, editor de imágenes de la prestigiosa revista británica Sunday Times. También tendremos al ganador absoluto de 2010, Chris Burkard, confirmado como juez, así como al legendario Fred Mortagne, el primer ganador absoluto de la Red Bull Illume Image Quest.
        <br /><br />
        «Para mí, este concurso cambió mi vida y mi carrera, el mero hecho de ser reconocido como fotógrafo fue alucinante. Como juez, busco imágenes que muestren cómo la experiencia humana se mezcla con estos entornos naturales salvajes en los que jugamos. También me entusiasma ver la creatividad llevada al límite y no puedo esperar a ver lo que la gente pone sobre la mesa». Dice Chris Burkard.
        <br /><br />
               
        </p>

      </div>

      <div class="article-cardn">
      <h1>Cuarta edición del Fotomatón Festival</h1>
      <img src="/assets/images/noticia11.png" alt="Notice 11" />
        <p className="primary-text">
     
        <br /><br />
        Las exposiciones este año serán:
        <br /><br />
        “The End” del estadounidense Rodney Smith cuyo humor, distinción y surrealismo exquisito le hicieron mundialmente famoso.
        <br /><br />
        “Disparando con el corazón”, una retrospectiva del trabajo de la que fue la primera fotoperiodista de nuestro país, Joana Biarnés. Personajes desde los Beatles a Dalí pasaron por el objetivo de esta sorprendente mujer.
        <br /><br />
        “Como si no hubiera un mañana” de Ricardo Cases y Martinho Costa. Esta exposición es una idea del fotógrafo Ricardo Cases cuya propuesta expositiva indaga sobre las limitaciones de la fotografía y la pintura como elementos de confrontación de la realidad.
        <br /><br />
        La cuarta exposición reflejará la obra ganadora del Portfolio Fotomatón en su segunda edición y en donde el autor premiado podrá exponer hasta un total de 25 obras.
                
        </p>

      </div>

      <div class="article-cardn">
      <h1>Apertura Inscripciones Premios LUX 2021</h1>
      <img src="/assets/images/noticia12.png" alt="Notice 12" />
        <p className="primary-text">
     
        <br /><br />
        La Asociación de Fotógrafos Profesionales de España, AFPE, lanza la 29ª edición de los Premios de Fotografía Profesional LUX y presenta la nueva Imagen LUX 2021, cuya autora es la Asociada Irene Sekulic.
        <br /><br />
        La convocatoria está abierta a todos los fotógrafos profesionales que ejerzan la profesión en el territorio español hasta el 1 de octubre.
        <br /><br />
        Después de batir récords de inscripciones durante los últimos años, los Premios LUX siguen consolidándose año tras año como un referente de la Fotografía Profesional en España.
                
        <br /><br />
               
        </p>

      </div>

      <div class="article-cardn">
      <h1>BFoto cierra su octava edición apoyando la difusión de los artistas emergentes</h1>
      <img src="/assets/images/noticia13.png" alt="Notice 13" />
        <p className="primary-text">
     
        <br /><br />
        La sobrina responsable del cuidado de Don Quijote, aquella muchacha que “no llegaba a los veinte” con un IPhone y una pompa de chicle o un grito de “mil veces no” tatuado en una pared al paso del río Vero son algunos de los rastros que ha dejado el Festival de Fotografía Emergente en las calles de Barbastro (BFoto) tras este fin de semana. Este encuentro, que apuesta por la gente que cree en lo que hace y quiere contarlo, ha celebrado el grueso de sus actividades durante los pasados 19,20 y 21 de agosto mediante la organización de exposiciones, charlas, un cine de verano y dos catas de vino del Somontano.
        <br /><br />
        Durante estas jornadas, el certamen se ha desarrollado con enorme participación ciudadana y ha convertido a la capital de la comarca oscense en un epicentro cultural donde los amantes de la fotografía han podido encontrar una gran variedad de iniciativas fotográficas. Por ello, la Asociación Fotográfica y de la Imagen de Barbastro (AFIB) – impulsora y organizadora del evento – hace un balance “plenamente satisfactorio” de este arranque del festival. Asimismo resaltan el aumento de propuestas recibidas durante esta edición: un total de 251 trabajos, de 40 nacionalidades diferentes en la categoría Emergentes y otros 149 en el apartado Open-Gran Formato.
                
        <br /><br />
               
        </p>

      </div>


      <div class="article-cardn">
      <h1>¡5 días! Red Bull Illume Image Quest 2021</h1>
      <img src="/assets/images/noticia14.png" alt="Notice 14" />
        <p className="primary-text">
     
        <br /><br />
        Red Bull Illume Image Quest 2021! © Michael Clark / Red Bull Illume
        <br /><br />
        Esta es la última llamada a todos los fotógrafos y videógrafos profesionales o aficionados de deportes de acción y aventura para que envíen sus trabajos.
        <br /><br />
        El plazo de presentación se acerca rápidamente: queda menos de una semana para que subas tus mejores fotos y vídeos antes del 31 de julio, ya sea a través de redbullillume.com o de Instagram utilizando el hashtag #rbi21submission y etiquetando a @redbullillume.
                
        <br /><br />
               
        </p>

      </div>


      <div class="article-cardn">
      <h1>Red Bull Illume: Medio mes para el final</h1>
      <img src="/assets/images/noticia15.png" alt="Notice 15" />
        <p className="primary-text">
     
        <br /><br />
        La cuenta atrás para el Red Bull Illume Image Quest 2021 ha comenzado, ya que se acerca el final de la fase de presentación de candidaturas. Solo queda medio mes para que los fotógrafos y videógrafos presenten sus mejores imágenes de aventura y deportes de acción para ganar prestigio, reconocimiento mundial, premios valorados en más de 200.000 euros y la oportunidad de que sus imágenes se publiquen en todo el mundo.
        <br /><br />
        El recordatorio de la fecha límite llega mientras revelamos algunos de los jueces que han aceptado ayudar a seleccionar a los ganadores. Entre ellos se encuentra Russ O’Connell, editor de imágenes de la prestigiosa revista británica Sunday Times. También tendremos al ganador absoluto de 2010, Chris Burkard, confirmado como juez, así como al legendario Fred Mortagne, el primer ganador absoluto de la Red Bull Illume Image Quest.
        <br /><br />
        «Para mí, este concurso cambió mi vida y mi carrera, el mero hecho de ser reconocido como fotógrafo fue alucinante. Como juez, busco imágenes que muestren cómo la experiencia humana se mezcla con estos entornos naturales salvajes en los que jugamos. También me entusiasma ver la creatividad llevada al límite y no puedo esperar a ver lo que la gente pone sobre la mesa». Dice Chris Burkard.
        <br /><br />
               
        </p>

      </div>

    </div>
    </div>
  );
};

export default Newsdetails2;